...
@section('content')
            ...
            <nav class="px-2">
                ...
                <ol class="breadcrumb d-flex text-capitalize">
                    <li class="breadcrumb-item"><a href="{{ URL::to(@$storeinfo->slug) }}"
                            class="text-white">{{ trans('home') }}</a></li>
                    <li class="breadcrumb-item active {{ session()->get('direction') == 2 ? 'breadcrumb-rtl' : '' }}">
                        {{ trans('labels.favorites') }}</li>
                </ol>
            </nav>
        </div>
    </div>
                                            ...
                                            <div class="card h-100 position-relative">
                                                <div class="overflow-hidden theme1grid_image">
                                                    <img src="@if (@$itemdata['item_image']->image_url != null) {{ @$itemdata['item_image']->image_url }} @else {{ helper::image_path($itemdata->image) }} @endif"
                                                        alt="" class="p-2 p-md-3 rounded-5"
                                                        onclick="showitems('{{ $itemdata->id }}','{{ $itemdata->item_name }}','{{ $itemdata->item_price }}','{{app()->getLocale()}}')">
                                                </div>
                                                <div class="card-body p-2 p-md-3 pb-0 pb-md-3">
                                                    @if (Auth::user() && Auth::user()->type == 3)
                                                        <div class="favorite-icon set-fav1-{{ $itemdata->id }}">
                                                            <a href="javascript:void(0)"
                                                                onclick="removefavorite('{{ $storeinfo->id }}','{{ $itemdata->id }}',0,'{{ URL::to($storeinfo->slug . '/managefavorite') }}')"><i
                                                    ...
                                                    <a href="javascript:void(0)" class="title pb-1"
                                                        onclick="showitems('{{ $itemdata->id }}','{{ $itemdata->item_name }}','{{ $itemdata->item_price }}','{{app()->getLocale()}}')">{{ $itemdata->item_name }}</a>
                                                    ...
                                                    <div class="row justify-content-between align-items-center gx-0">
                                                        ...
                                                        <div class="col-3 col-md-8 d-flex justify-content-end mb-2 mb-md-0">
                                                            @if ($itemdata->has_variants == 1)
                                                                <button class="btn-primary product-cart-icon" type="button"
                                                                    onclick="showitems('{{ $itemdata->id }}','{{ $itemdata->item_name }}','{{ $itemdata->item_price }}','{{app()->getLocale()}}')">
                                                            ...
                                                            @else
                                                                ...
                                                                <button
                                                                    class="btn-primary product-cart-icon addcartbtn-{{ $itemdata->id }}"
                                                                    type="button"
                                                                    onclick="addtocart('{{ $itemdata->id }}','{{ $itemdata->item_name }}','{{ $itemdata->item_price }}','{{ $image }}','{{ $itemdata->tax }}','1','{{ $itemdata->item_price }}')">
                                                                    <i class="fa-solid fa-cart-shopping"></i>
                                                                </button>
                                                            @endif
                                                        </div>
                                    ...
                                    <div class="d-flex justify-content-center align-items-center m-auto mt-4">
                                        <nav aria-label="Page navigation example">
                                            @if ($getfavoritelist->lastPage() > 1)
                                                <ul class="pagination">
                                                    <li
                                                        class="page-item {{ $getfavoritelist->currentPage() == 1 ? ' disabled' : '' }}">
                                                        <a class="page-link {{ session()->get('direction') == 2 ? 'rounded-end rounded-start-0' : 'rounded-start rounded-end-0' }}"
                                                            href="{{ $getfavoritelist->url(1) }}" aria-label="Previous">
                                                            <span aria-hidden="true">&laquo;</span>
                                                        </a>
                                                    </li>
                                                    @for ($i = 1; $i <= $getfavoritelist->lastPage(); $i++)
                                                        <li
                                                            class="page-item  {{ $getfavoritelist->currentPage() == $i ? ' active' : '' }}">
                                                            <a class="page-link"
                                                                href="{{ $getfavoritelist->url($i) }}">{{ $i }}</a>
                                                        </li>
                                                    @endfor
                                                    <li
                                                        class="page-item {{ $getfavoritelist->currentPage() == $getfavoritelist->lastPage() ? ' disabled' : '' }}">
                                                        <a class="page-link {{ session()->get('direction') == 2 ? 'rounded-start rounded-end-0' : 'rounded-end rounded-start-0' }}"
                                                            href="{{ $getfavoritelist->url($getfavoritelist->currentPage() + 1) }}"
                                                            aria-label="Next">
    ...
    <button class="btn account-menu btn-primary d-lg-none d-md-block hide_when_footer_bar_show" type="button"
